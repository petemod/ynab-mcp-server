...
import { z } from "zod";
class SearchTransactionsTool extends MCPTool {
    name = "search_transactions";
    description = "Search transactions by memo or payee name (excluding transfers) with fuzzy matching. Returns most relevant results.";
    schema = {
        budgetId: {
    ...
    async execute(input) {
        ...
        try {
            ...
            const response = await this.api.transactions.getTransactions(budgetId, input.sinceDate);
            const allTransactions = response.data.transactions || [];
            // Filter out deleted transactions
            const activeTransactions = allTransactions.filter((t) => !t.deleted);
            // Search and score transactions
            const searchResults = this.searchAndScoreTransactions(activeTransactions, input.searchText.toLowerCase());
            ...
            return `Error searching transactions: ${error instanceof Error ? error.message : JSON.stringify(error)}`;
        }
    }
    searchAndScoreTransactions(transactions, searchText) {
        const results = [];
    ...
    calculateMatchScore(text, searchText) {
    ...
    calculateOverlap(text, searchText) {
