...
import { z } from "zod";
class UpdateScheduledTransactionTool extends MCPTool {
    ...
    async execute(input) {
        ...
        const milliunitAmount = input.amount !== undefined ? Math.round(input.amount * 1000) : undefined;
        try {
            ...
            const response = await this.api.scheduledTransactions.updateScheduledTransaction(budgetId, input.scheduledTransactionId, scheduledTransaction);
            ...
            logger.error(`Error updating scheduled transaction ${input.scheduledTransactionId} for budget ${budgetId}:`);
